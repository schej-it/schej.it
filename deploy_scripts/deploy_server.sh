...
ssh $SERVER_HOST -i $AWS_KEY_LOCATION "cd $ROOT_FOLDER_SERVER_LOCATION && sudo git stash && sudo git pull && cd server && sudo go build -buildvcs=false && screen -XS schej.it-server quit; screen -dmS schej.it-server ./server"
echo "Done!"
